<?
  ...
  $count=mysql_num_rows(mysql_query("SELECT*FROM binhchon"));
  ...
  if(isset($_REQUEST['list_id']) && $_REQUEST['list_id'] != ''){
		...
		for($i = 0; $i < count($del_pic_id); $i++){
			...
			$sql = "SELECT * FROM binhchon WHERE id = '" . $del_pic_id[$i] . "'";
		...
		mysql_query('DELETE FROM binhchon WHERE binhchon.id IN(' . $list_id . ')');

		?>

	   <script>window.location ="quan_tri.php?p=binhchon"</script>
	...
	$noibat=$_GET['trangthai'];

	$id=$_GET['id'];

	if($noibat==0){

	 mysql_query("update binhchon set trangthai=1 where id=".$id."");

	}

	if($noibat==1){

	 mysql_query("update loaiphong set trangthai=0 where id=".$id."");
...
function delete_all_chosen(){
	...
	window.location ="quan_tri.php?p=danhsach_loaiphong&list_id=" + list_id;
...

	{
...

		{
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=binhchon&page=1\" title=\"Trang đầu\" style='margin:4px;'> << </a>";
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=binhchon&page=".($curpage-1)."\" title=\"Về trang trước\" style='margin:4px;'><font color='#00ccff'> < </font></a>";
...

			{
...

				{
					...
					$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=binhchon&page=".$i."\" title=\"Trang ".$i."\" style='margin:4px;'><font color='red'>[".$i."]</font></a>";
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=binhchon&page=".($curpage+1)."\" title=\"Đến trang sau\" style='margin:4px;' ><font color='#00ccff'> > </font></a>";
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=binhchon&page=".$pages."\" title=\"Trang cuối\" > >> </a>";
...

		{
...

			{
				...
				$next_prev="<a href=\"?p=binhchon&page=".($curpage-1)."\">Back</a>";
...

			{
				...
				$next_prev="<a href=\"?p=binhchon&page=".($curpage+1)."\">Next</a>";
...
<?php
...

	{
	  ...
	  $result=mysql_query("SELECT * FROM binhchon $loai ORDER BY id DESC limit $start,$limit");
     ...
     echo "<h2 align='center'>Danh Sách Bình Chọn </h2>";

		  ?>

          <form action="quan_tri.php?p=binhchon" method="post">
...
<a href="#"  style="float:left;margin-right:10px; width: 100px;" 
    ...
    <?
		...
		//echo "<td >Sạch sẽ</td>";
	     ...
	     //echo "<td >Tiện nghi</td>";

	     //echo "<td >Giá</td>";

	     //echo "<td >Chất lượng</td>"; 
	   ...
	   while($row = mysql_fetch_object($result))
		 ...
		 $did=$row->thuocloai;
	      ...
	      $sql3=mysql_query("select * from loaiphong where id='".$did."'");
...
?>
		...
		<a href="quan_tri.php?p=traloibinh&id=<?=$id?>">Trả lời</a>
		...
		<a href="quan_tri.php?p=binhchon&trangthai=0&id=<?=$id?>">No</a>
		...
		<a href="quan_tri.php?p=binhchon&trangthai=1&id=<?=$id?>">Active</a>
...
<?
			...
			echo  "<td align='center'><a href=\"quan_tri.php?p=xoa_binhchon&id=".$id."\" ­ onclick=\"return confirm('Bạn có muốn xóa thông tin này ?')\"><img src='images/xoa_record.png' border='0'></a></td>";

		echo  "<td align='center'><a href='quan_tri.php?p=sua_binhchon&id=".$id."&khachsan=".$khachsan."'><img src='images/sua_record.png' border='0'></a></td>";
