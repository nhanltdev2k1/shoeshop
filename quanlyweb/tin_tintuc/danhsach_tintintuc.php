...
if (isset($_REQUEST['list_id']) && $_REQUEST['list_id'] != '') {
	...
	for ($i = 0; $i < count($del_pic_id); $i++) {
		$sql = "SELECT * FROM tin_tintuc WHERE id = '" . $del_pic_id[$i] . "'";
		mysqli_query($sql);
		//$pic = $d->result_array();

	}
	mysql_query('DELETE FROM tin_tintuc WHERE tin_tintuc.id IN(' . $list_id . ')');
?>
	<script>
...
if (mysqli_num_rows($result) <> 0) {
	...
	while ($row = mysqli_fetch_object($result)) {
		...
		// $did=$row->thuocloai;
		//$sql3=mysql_query("select * from loai_tin_tintuc where id='".$did."'");
